...
```
This Windows API retrieve the fullpath of the specified module, the return value being the length of the string copied to the buffer (lpFileName)
```
...
You will then be able to deduct that:
```
...
We see that in 0x00401668, the eax is holding the hkey **\"HKLM\\SOFTWARE\\Dreamer\"**. If the RegCreateKeyA is successful, we will go to loc_401677, where eax will be phkResult. Looking at MSDN of RegCreateKeyA, we know that: "A pointer to a variable that receives a handle to the opened key. If the key is not one of the predefined registry keys". Hence we can be confident that RegSetValueExA will set a value in **\"HKLM\\SOFTWARE\\Dreamer\"**.

Why is the key HKLM? Look at the **RegCreateKeyA** at base+1668 and see the hkey parameter passed in as 800000002h. This is HKLM. 
...
What value is this key gonna store? MSDN TIME 

We need to note in the param:
...
So we now know that:
```
The function will set the value default (since lpValueName is null) of key SOFTWARE\Dreamer to 4 bytes pointed to by Data. 
```

At this point i start to question what is inside Data... Highlight it shows that it's value is a local variable initialized to 0. There is no operation where it is on the dest operands. So that leaves the possibility that one of the API actually load the data from somewhere. Which brings us here: 
...
**So, stringing these 3 API together:**
...
* Else if, the check the key "SOFTWARE\Dreamer" and if we can open that key, then Query the value into local variable Data.
* Else, put data = 1 (.text:0x00401530)

Now this will make you think what is inside the file is probably _just an integer_. Else why its default case is 1 ??? This also tallies with the fact that we only load in 4 bytes -- size of an int. 

The significance of this value? We highlight the variable and scroll **DOWN** until we see something interesting: 
* The value got written into "HKLM\SOFTWARE\Dreamer"  -- probably because the key didn't exist, yet.
...
```
    ...
    LocalSystem account.
```
This suggests some sort of persistence mechanism (only copy the file to somewhere safe when system reboots). 
...
We see that after all the hoo-haas about getting the integer value Data in part e, it all goes to this portion, aka this portion is dependent of all the if-elses above. Now what Module is specified by 0? According to MSDN:
```
...
With that, we can see that the file will delete itself (the original position). By now we should all know that it probably has copied itself somewhere else, so let's look at CopyFileA API where DST itself is lpExitingFileName: 

![idest](./Assets/idest.png)

HMMMMMMMMMM interesting so Dest is the new copy, and dest comes from: 

![idest](./Assets/idest2.png)

[GetSystemDirectoryA](https://docs.microsoft.com/en-us/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemdirectorya) will get the "C:Windows\System32". This is the first part of the new file (the dir part), we still need the filename. So scrolling around and we see a bunch of str related function, and a variable Str that is used with strcat (base+15CE) on Dest. 

**So what is Str?**

![istrchr](./Assets/istrchr.png)

[strrchr](http://www.cplusplus.com/reference/cstring/strrchr/) returns the last occurence of a character. The character is 5C in hexa -- aka the "\\". In Dest variable, we know it has the fullpath to the malware. We know after the last slash is the malware name. 
...
_\*This is my host machine shell:personal folder_ 

We see that shell:personal point to **Documents**. By RegQueryValueA MSDN, we know the resulting path will be stored in lpData at local var_E54. 

_At this stage, my browser is looking like this_
...
* hFile is var_C, which was created from CreateFileA which was 4069.txt 
...
Now it would be a lot simpler to look at the [_horrendous_](https://sites.google.com/a/swivvl.com/programming-projects/traverse-directory) source code. But after reading it you will make sense of the random string lying around like "\\*" (base+12A2) -- which is a wild card character for all the files in the directory. Why would you need wild card? MSDN of FindFirstFileA says: "Searches a directory for a file or subdirectory with a name that matches a specific name (or partial name if wildcards are used)." 
