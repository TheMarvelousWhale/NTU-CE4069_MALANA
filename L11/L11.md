...
1. Do and Don't of Malware Analysis
2. Static -- PE Header, Packing, Obfuscation
3. Dynamic -- Launching, Dynamic analysos
5. Persistence
6. Assemblies Reversing
7. Windows API
...
* Document results and findings along the way, taking snapshots as necessary.
...
*Strings*
* strings can be ascii, unicode or other locale strings. strings are big tell-tale sign and often (but not always) good host indicator / indicator of packed/encrypted malware
...
* click on it if exe
* rundll.exe or ollydbg it if dll
...
* is usually seed | generator function(seed) | TLD set
* How to blocK: Filter, Score domain name, Pattern Matching, RE
* Types of DGA: Arithmetic, wordlist, hash based, permutation based

## 4. Persistence

10 techniques:

1. Run registry: \SOFTWARE\Microsoft\Windows\CurrentVersion\Run. To survive reboot. 
2. Winlogon: HKEY SHell, Userinit, SOFTWARE\Microsoft\Windows. To start with winlogon. 
...
6. Startup folder: HKEY Windows\Start Menu\Programs\Startup. I used this folder to open my google calendar upon startup. It's pretty neat. 
7. Scheduled task: cron (unix) or schtasks. This is mainly to allow malware to be executed periodically (e.g keep heartbeat to C2)
...
9. Dll Search order hijacking: mess with the lookup process of a dll name to load a malicious dll
...
* Function call: internall call \<\name> vs import call ds:\<name in IAT\>
...
*Network*:
* WSA - Startup > Socket > Connect for client 
* WSA - Startup > Socket > Bind > Listen for server
...
5. Revershell: tie STARTUPINFO to Winsock and CreateProcess (assignment 2)
OR multithread that with 2 pipes and 2 threads
...
12. HashDump: GetTheHash via mimikatz to authenticate as admin inside a system. Hash are often LTM or NLTM hashes stored in SAM (lsass.exe, lsaext.dll)
...
As Prof Tang Xueyan said in CE4013 Distributed System, nothing can replace a full and thorough read through of your materials. What I write is just pointer to focus on more if you do not have enough time. All the best for exam  	(((o(*°▽°*)o)))

Finally, as everything good has to come an end, I guess it's time to say goodbye. 
