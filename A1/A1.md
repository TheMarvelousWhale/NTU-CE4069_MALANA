...
Kevin Li has reported that he has the fakenet result for those IP after setting the default browser to IE. I was too lazy at this point (26/02/21) to insert it here. 
...
Now you may ask **why the malware behave like this?** The hypothesis me and Kevin Li and Simp Songqin have developed are as followed:
   ...
   3. After repeating many different configurations (different HKEY orders, different browsers availability), we noticed the following:
      ...
      2. Kevin uninstalled Chrome from the given OVA. It still attack Chrome.exe. This indicated that it based its browser search from HKEY or some other env var that wasn't cleaned up properly. 
