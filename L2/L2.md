...
The filters I used for procmon in this tutorial are (**click on the filter symbol on top pane**)
```
...
```
It is to study the process charles, funny.exe and the file charles created. The full procmon output is also here in [pml native format](./charles_procmon_log.PML) and [csv](./charles_procmon_log.csv).
...
it is just whether you can sift out the info anot
```
...
```
We see that one value in HKEY_USERShas been modified
Key: SOFTWARE\Microsoft\Windows\CurrentVersion\Run\CECZ4069
Value:"C:\Users\hoang\AppData\Roaming\naughtyme\funny.exe" 
```

Or you can be a dulan and use ipynb like a true idiot like me
...
Or... i feel like the idiocy is not enough so I will walk through **every single line of binary code to find the mutex in ollydbg**
